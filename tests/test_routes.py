...
def test_pause_habit_success(logged_in_client, app):
    """Test that POST /habit-tracker/pause/<id> pauses a habit."""
    with app.app_context():
        from extensions import db

        habit = Habit(name="Morning Run", description="Daily running")
        db.session.add(habit)
        db.session.commit()
        habit_id = habit.id
...
def test_pause_habit_requires_auth(client, app):
    """Test that pausing a habit requires authentication."""
    with app.app_context():
        from extensions import db
...
def test_pause_habit_invalid_id_returns_404(logged_in_client):
    """Test that pausing an invalid habit ID returns 404."""
    response = logged_in_client.post("/habit-tracker/pause/99999", follow_redirects=False)

    assert response.status_code == 404


def test_resume_habit_success(logged_in_client, app):
    """Test that POST /habit-tracker/resume/<id> resumes a paused habit."""
    with app.app_context():
        from datetime import datetime, timezone
...
def test_resume_habit_requires_auth(client, app):
    """Test that resuming a habit requires authentication."""
    with app.app_context():
        from extensions import db

        habit = Habit(name="Test Habit", is_paused=True)
        db.session.add(habit)
        db.session.commit()
        habit_id = habit.id
...
def test_resume_habit_invalid_id_returns_404(logged_in_client):
    """Test that resuming an invalid habit ID returns 404."""
    response = logged_in_client.post("/habit-tracker/resume/99999", follow_redirects=False)

    assert response.status_code == 404


def test_habit_tracker_shows_paused_habits_separately(logged_in_client, app):
    """Test that the habit tracker page displays paused habits in a separate section."""
    with app.app_context():
        from datetime import datetime, timezone

        from extensions import db

        active_habit = Habit(name="Active Habit", description="Active")
        paused_habit = Habit(
            name="Paused Habit",
            description="Paused",
            is_paused=True,
    ...
    assert "Paused Habits" in html  # Section title


def test_paused_habit_independent_of_archive(logged_in_client, app):
    """Test that pause and archive are independent features."""
    with app.app_context():
        from extensions import db
    ...
    # Pause the habit
    logged_in_client.post(f"/habit-tracker/pause/{habit_id}")

    # Archive the paused habit
    logged_in_client.post(f"/habit-tracker/archive/{habit_id}")

    with app.app_context():
        habit = Habit.query.filter_by(id=habit_id).first()
        assert habit.is_paused is True  # Still paused
        assert habit.is_archived is True  # Also archived
        # Both states can coexist independently
